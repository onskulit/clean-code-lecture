    ...
    const validate = (type, value) => {
        switch (type) {
            ...
            case 'surname':
                ...
                } else if (!regexSurname.test(value)) return `Фамилия должна содержать только буквы русского алфавита`;
            ...
            case `grade`:
                if (!value || value > FIVE || value < ONE || !parseInt(value)) {
                    ...
                    } else if (value > 5) return 'Оценка не должна быть больше 5';
                    else if (value < 1) {
...
document.querySelector('form').addEventListener('submit', function (e) {
    ...
    const item = elementCreator(
        'li',
        ['list-item', isNotGoodGrade === true ? `list-item_yellow` : undefined].filter((c) => Boolean(c)),
        undefined,
        list
    );
